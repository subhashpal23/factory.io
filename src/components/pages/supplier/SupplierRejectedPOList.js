...
import { getRejectedSupplierPOList, getProductList, getTaxCategoryList } from '../../../redux/actions/rfqAction';
...
const SupplierRejectedPOList = () => {
  const dispatch = useDispatch();
  const { logindata, loginError } = useSelector((state) => state.auth);
  const { supplierRejectedPoData, error , taxCategoryData} = useSelector((state) => state.rfq); 
  ...
  const rfqList = supplierRejectedPoData && supplierRejectedPoData?.data ? supplierRejectedPoData.data : []
 
  useEffect(() => {
    if (logindata && logindata.token) {
      dispatch(getRejectedSupplierPOList(logindata.token));
...
export default SupplierRejectedPOList;